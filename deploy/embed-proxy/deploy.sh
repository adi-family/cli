#!/usr/bin/env bash
# Deploy embed-proxy to GCP Cloud Run
# Usage: ./deploy.sh
set -euo pipefail
...
IMAGE="${GCP_REGION}-docker.pkg.dev/${GCP_PROJECT}/${REPO_NAME}/embed-proxy"
...
DATABASE_URL="${DATABASE_URL}" embed-proxy-migrate 2>/dev/null || \
  echo "   (skipped — embed-proxy-migrate not found locally, migrations will run in container)"

echo "==> Building embed-proxy image (tag: $TAG, platform: linux/amd64)"
docker build \
