#!/usr/bin/env bash
# Deploy registry to GCP Cloud Run
# Usage: ./deploy.sh
set -euo pipefail
...
IMAGE="${GCP_REGION}-docker.pkg.dev/${GCP_PROJECT}/${REPO_NAME}/registry"
...
echo "==> Building registry image (tag: $TAG, platform: linux/amd64)"
docker build \
