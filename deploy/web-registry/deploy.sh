#!/usr/bin/env bash
# Deploy Web registry to GCP Cloud Run
# Usage: ./deploy.sh
set -euo pipefail
...
IMAGE="${GCP_REGION}-docker.pkg.dev/${GCP_PROJECT}/${REPO_NAME}/web-registry"
...
echo "==> Building Web registry image (tag: $TAG, platform: linux/amd64)"
docker build \
