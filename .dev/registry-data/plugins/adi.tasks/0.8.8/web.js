...
const x = "tasks", yt = (i, t) => i.request(x, "list", t ?? {}), _t = (i, t) => i.request(x, "create", t), vt = (i, t) => i.request(x, "get", { task_id: t }), kt = (i, t) => i.request(x, "update", t), xt = (i, t) => i.request(x, "delete", { task_id: t }), wt = (i, t, e) => i.request(x, "search", { query: t, limit: e }), X = (i) => i.request(x, "stats", {});
function z() {
 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
const V = globalThis, rt = (i) => i, L = V.trustedTypes, ot = L ? L.createPolicy("lit-html", { createHTML: (i) => i }) : void 0, gt = "$lit$", y = `lit$${Math.random().toFixed(9).slice(2)}$`, ft = "?" + y, Rt = `<${ft}>`, k = document, D = () => k.createComment(""), O = (i) => i === null || typeof i != "object" && typeof i != "function", Z = Array.isArray, Ht = (i) => Z(i) || typeof i?.[Symbol.iterator] == "function", F = `[ 	
\f\r]`, T = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, nt = /-->/g, at = />/g, _ = RegExp(`>|${F}(?:([^\\s"'>=/]+)(${F}*=${F}*(?:[^ 	
...
const Bt = (i, t) => {
  ...
  for (let l = 0; l < e; l++) {
    const a = i[l];
    let c, h, d = -1, g = 0;
    for (; g < a.length && (n.lastIndex = g, h = n.exec(a), h !== null); ) g = n.lastIndex, n === T ? h[1] === "!--" ? n = nt : h[1] !== void 0 ? n = at : h[2] !== void 0 ? ($t.test(h[2]) && (r = RegExp("</" + h[2], "g")), n = _) : h[3] !== void 0 && (n = _) : n === _ ? h[0] === ">" ? (n = r ?? T, d = -1) : h[1] === void 0 ? d = -2 : (d = n.lastIndex - h[2].length, c = h[1], n = h[3] === void 0 ? _ : h[3] === '"' ? ct : lt) : n === ct || n === lt ? n = _ : n === nt || n === at ? n = T : (n = _, r = void 0);
    const b = n === _ && i[l + 1].startsWith("/>") ? " " : "";
    o += n === T ? a + Rt : d >= 0 ? (s.push(c), a.slice(0, d) + gt + a.slice(d) + y + b) : a + y + (d === -2 ? l : b);
  }
    ...
    for (; (r = v.nextNode()) !== null && a.length < l; ) {
      if (r.nodeType === 1) {
        if (r.hasAttributes()) for (const d of r.getAttributeNames()) if (d.endsWith(gt)) {
          const g = h[n++], b = r.getAttribute(d).split(y), N = /([.?@])?(.*)/.exec(g);
