adi-balance, rust, axum, postgres, balance, transactions, credits
...
```bash
# Run all migrations
DATABASE_URL=postgres://... cargo run --bin balance-migrate all

# Check migration status
DATABASE_URL=postgres://... cargo run --bin balance-migrate status

# Dry run (show pending)
DATABASE_URL=postgres://... cargo run --bin balance-migrate dry-run
```
