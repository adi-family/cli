...
use credentials_core::{
    ApiError, CreateCredential, Credential, CredentialAccessLog, CredentialRow, CredentialWithData,
...
) -> HttpResult<Json<Vec<Credential>>> {
...
pub async fn get_with_data(
    ...
    user: AuthUser,
    Path(id): Path<Uuid>,
) -> HttpResult<Json<CredentialWithData>> {
    let row = sqlx::query_as::<_, CredentialRow>(
        "SELECT * FROM credentials WHERE id = $1 AND user_id = $2",
    )
    .bind(id)
...
pub async fn create(
    ...
    if input.name.trim().is_empty() {
        return Err(ApiError::BadRequest("Name cannot be empty".to_string()).into());
    }
...
pub async fn get_access_logs(
    ...
    user: AuthUser,
    Path(id): Path<Uuid>,
) -> HttpResult<Json<Vec<CredentialAccessLog>>> {
