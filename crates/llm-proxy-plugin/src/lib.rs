...
impl Plugin for LlmProxyPlugin {
    fn metadata(&self) -> PluginMetadata {
        PluginMetadata {
            id: "adi.llm-proxy".to_string(),
            name: "ADI LLM Proxy".to_string(),
...
impl CliCommands for LlmProxyPlugin {
    ...
    async fn run_command(&self, ctx: &CliContext) -> PluginResult<CliResult> {
        ...
        let result = match subcommand {
            ...
            "" | "help" => Ok(get_help()),
            _ => Err(format!(
                "Unknown command: {}. Use 'llm-proxy help' for usage.",
                subcommand
            )),
        };
...
fn get_help() -> String {
    ...
    help.push_str("ADI LLM Proxy - LLM API proxy with BYOK/Platform modes\n\n");
    help.push_str("USAGE:\n");
    help.push_str("  adi llm-proxy <COMMAND>\n\n");
    ...
    help.push_str("  adi llm-proxy keys list\n");
    help.push_str(
        "  adi llm-proxy tokens create --name \"my-token\" --mode byok --key-id <uuid>\n",
    );
    help.push_str("  adi llm-proxy usage --from 2024-01-01\n");
    help
}
