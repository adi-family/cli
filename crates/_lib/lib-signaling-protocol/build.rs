...
fn main() {
    ...
    let source = std::fs::read_to_string("signaling.tsp").expect("read signaling.tsp");
    let file = typespec_api::parse(&source).expect("parse signaling.tsp");
    ...
    Generator::new(&file, &proto_dir, "signaling")
