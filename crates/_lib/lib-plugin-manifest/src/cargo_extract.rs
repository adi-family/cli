...
pub fn generate_manifest_from_cargo(
    ...
    // Daemon
    let daemon = parse_daemon(metadata_plugin);
...
fn parse_daemon(meta: &toml::Value) -> Option<DaemonInfo> {
    let daemon = meta.get("daemon")?;
    Some(DaemonInfo {
        auto_start: daemon
            .get("auto_start")
            .and_then(|v| v.as_bool())
            .unwrap_or(false),
        restart_on_failure: daemon
        ...
        max_restarts: daemon
            ...
            .map(|v| v as u32)
            .unwrap_or(3),
        restart_exponential_backoff: daemon
            .get("restart_exponential_backoff")
            .and_then(|v| v.as_bool())
            .unwrap_or(false),
    })
}
