...
pub struct PluginManifest {
    ...
    /// Web UI configuration (for plugins with browser-based UIs)
    #[serde(default)]
    pub web_ui: Option<WebUiInfo>,
...
impl PluginManifest {
    ...
    pub fn from_toml(content: &str) -> Result<Self, ManifestError> {
        let manifest: Self = toml::from_str(content).map_err(ManifestError::TomlParse)?;
        if let Some(ref web_ui) = manifest.web_ui {
            if web_ui.entry.is_empty() {
                return Err(ManifestError::InvalidFormat(
                    "web_ui.entry must not be empty".to_string(),
                ));
            }
        }
        Ok(manifest)
...
/// Web UI configuration for plugins with browser-based interfaces.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct WebUiInfo {
    /// Single JS entry point filename (e.g., "index.js")
    pub entry: String,

    /// Whether to iframe-sandbox the plugin (default: false)
    #[serde(default)]
    pub sandbox: bool,
}

/// Daemon service metadata.
///
/// Declares that this plugin provides a background daemon service
/// managed by `adi daemon`.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DaemonInfo {
    /// Start automatically when adi daemon starts
    #[serde(default)]
    pub auto_start: bool,
    ...
    /// Use exponential backoff between restart attempts
    #[serde(default)]
    pub restart_exponential_backoff: bool,
}
...
impl Default for DaemonInfo {
    fn default() -> Self {
        Self {
            ...
            max_restarts: 3,
            restart_exponential_backoff: false,
        }
    }
}
...
impl PluginManifest {
    ...
    pub fn to_toml(&self) -> Result<String, ManifestError> {
        toml::to_string_pretty(self)
            .map_err(|e| ManifestError::InvalidFormat(format!("Failed to serialize manifest: {e}")))
    }
}
...
"#;
    ...
    #[test]
    fn test_parse_manifest_without_web_ui() {
        let toml = r#"
[plugin]
id = "adi.tasks"
name = "ADI Tasks"
...
"#;
    ...
    #[test]
    fn test_parse_manifest_with_web_ui() {
        let toml = r#"
[plugin]
id = "adi.tasks.web"
name = "ADI Tasks Web UI"
version = "1.0.0"
type = "web"
...
"#;
        let manifest = PluginManifest::from_toml(toml).unwrap();
        let web_ui = manifest.web_ui.unwrap();
        assert_eq!(web_ui.entry, "index.js");
        assert!(web_ui.sandbox);
    }

    #[test]
    fn test_parse_manifest_with_web_ui_minimal() {
        let toml = r#"
[plugin]
id = "adi.tasks.web"
name = "ADI Tasks Web UI"
...
"#;
    ...
    #[test]
    fn test_web_ui_roundtrip_serialization() {
        let toml_input = r#"
[plugin]
id = "adi.tasks.web"
name = "ADI Tasks Web UI"
...
"#;
        ...
        let serialized = manifest.to_toml().unwrap();
        let reparsed = PluginManifest::from_toml(&serialized).unwrap();
        assert_eq!(reparsed.web_ui, manifest.web_ui);
        let web_ui = reparsed.web_ui.unwrap();
        assert_eq!(web_ui.entry, "index.js");
        assert!(web_ui.sandbox);
    }

    #[test]
    fn test_web_ui_empty_entry_rejected() {
        let toml = r#"
[plugin]
id = "adi.tasks.web"
name = "ADI Tasks Web UI"
...
"#;
        ...
        assert!(
            err.contains("web_ui.entry must not be empty"),
            "Error was: {err}"
        );
    }
