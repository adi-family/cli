...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::registry()
        .with(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "adi_llm_proxy=debug,tower_http=debug".into()),
        )
    ...
    let app = Router::new()
        ...
        .nest("/api/llm-proxy", management_routes())
    ...
    // Start server
    tracing::info!("Starting adi-llm-proxy on {}", addr);
