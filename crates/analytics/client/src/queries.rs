...
) -> QueryResult<Vec<DailyActiveUsers>> {
...
pub async fn get_endpoint_latency(
    ...
    start_date: DateTime<Utc>,
    end_date: DateTime<Utc>,
) -> QueryResult<Vec<EndpointLatency>> {
...
pub async fn get_slowest_endpoints(pool: &PgPool) -> QueryResult<Vec<EndpointLatency>> {
    let rows = sqlx::query_as::<_, EndpointLatencyRow>(
        r#"
        SELECT
            hour,
...
pub async fn get_overview_stats(pool: &PgPool) -> QueryResult<OverviewStats> {
