...
const SERVICE_ID: &str = "adi.indexer.lang.rust";
...
extern "C" fn plugin_info() -> PluginInfo {
    ...
    .with_description("Rust language parsing and analysis for ADI indexer")
...
extern "C" fn plugin_init(ctx: *mut PluginContext) -> i32 {
    unsafe {
        ...
        // Register language analyzer service
        let descriptor =
            ServiceDescriptor::new(SERVICE_ID, ServiceVersion::new(1, 0, 0), "adi.lang.rust")
                .with_description("Rust language analyzer for code indexing");
        ...
        if let Err(code) = host.register_svc(descriptor, handle) {
            host.error(&format!(
                "Failed to register Rust analyzer service: {}",
                code
            ));
            return code;
        }

        host.info("Rust language plugin initialized");
    }
...
extern "C" fn analyzer_list_methods(_handle: *const c_void) -> RVec<ServiceMethod> {
    vec![
        ...
        ServiceMethod::new(METHOD_EXTRACT_SYMBOLS)
            .with_description("Extract symbols from Rust source code"),
        ServiceMethod::new(METHOD_EXTRACT_REFERENCES)
            .with_description("Extract references from Rust source code"),
...
fn handle_get_grammar_path() -> RResult<RString, ServiceError> {
    // For now, we use the built-in grammar from tree-sitter-rust crate
    // In a full plugin, this would point to a separate .so file
    let response = GrammarPathResponse {
        path: "builtin".to_string(), // Special value indicating built-in grammar
    };
