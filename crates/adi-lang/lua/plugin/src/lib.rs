...
const SERVICE_ID: &str = "adi.indexer.lang.lua";
...
extern "C" fn plugin_info() -> PluginInfo {
    ...
    .with_description("Lua language parsing and analysis for ADI indexer")
...
extern "C" fn plugin_init(ctx: *mut PluginContext) -> i32 {
    unsafe {
        let host = (*ctx).host();
        let descriptor =
            ServiceDescriptor::new(SERVICE_ID, ServiceVersion::new(1, 0, 0), "adi.lang.lua")
                .with_description("Lua language analyzer for code indexing");
        ...
        if let Err(code) = host.register_svc(descriptor, handle) {
            host.error(&format!(
                "Failed to register Lua analyzer service: {}",
                code
            ));
            return code;
        }
        host.info("Lua language plugin initialized");
    }
    0
}
...
extern "C" fn analyzer_list_methods(_handle: *const c_void) -> RVec<ServiceMethod> {
    vec![
        ...
        ServiceMethod::new(METHOD_EXTRACT_SYMBOLS)
            .with_description("Extract symbols from Lua source code"),
        ServiceMethod::new(METHOD_EXTRACT_REFERENCES)
            .with_description("Extract references from Lua source code"),
