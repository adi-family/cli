...
use adi_api_proxy_core::{db, ApiResult, ProviderType};
...
pub async fn update_platform_key(
    ...
    let key = if let Some(is_active) = req.is_active {
        ...
        keys.into_iter().find(|k| k.id == id).ok_or_else(|| {
            adi_api_proxy_core::ApiError::NotFound("Platform key not found".into())
        })?
    };
...
pub async fn verify_platform_key(
    ...
    let provider =
        adi_api_proxy_core::providers::create_provider(key.provider_type, key.base_url.clone());
