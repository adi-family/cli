...
use adi_api_proxy_core::{ApiError, ApiResult, ProxyRequest, RequestStatus};
    ...
    match result {
        ...
        Err(e) => {
            let (status, error_type) = match &e {
                adi_api_proxy_core::providers::ProviderError::AuthenticationFailed => {
                    (StatusCode::UNAUTHORIZED, "authentication_failed")
                }
                adi_api_proxy_core::providers::ProviderError::RateLimited => {
                    (StatusCode::TOO_MANY_REQUESTS, "rate_limited")
                }
                adi_api_proxy_core::providers::ProviderError::Timeout => {
    ...
    match stream_result {
        Ok(upstream_stream) => {
            ...
            tokio::spawn(async move {
                ...
                let log_result = adi_api_proxy_core::db::usage::log_usage(
                    state_clone.db.pool(),
                    token.id,
                    token.user_id,
                    &request_id,
