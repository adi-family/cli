...
use adi_api_proxy_core::{ApiError, ApiResult, ProviderType, ProxyRequest, RequestStatus};
...
pub async fn messages(
    ...
    let provider_type = match token.key_mode {
        adi_api_proxy_core::KeyMode::Byok => {
            let key_id = token.upstream_key_id.ok_or_else(|| {
                ApiError::Internal("BYOK token missing upstream_key_id".to_string())
            })?;
            let key = adi_api_proxy_core::db::keys::get_upstream_key_by_id(state.db.pool(), key_id)
                .await?;
            key.provider_type
        }
        adi_api_proxy_core::KeyMode::Platform => token.platform_provider.ok_or_else(|| {
    ...
    match stream_result {
        Ok(upstream_stream) => {
            ...
            tokio::spawn(async move {
                ...
                let mut usage_info: Option<adi_api_proxy_core::UsageInfo> = None;
                                            ...
                                            serde_json::from_str::<serde_json::Value>(data)
                                        {
                                                ...
                                                == Some("message_delta")
                                            {
                                                if let Some(usage) = json.get("usage") {
                                                    usage_info =
                                                        Some(adi_api_proxy_core::UsageInfo {
                                                            input_tokens: usage
                                                                .get("input_tokens")
                                                                .and_then(|v| v.as_i64())
                                                                .map(|v| v as i32),
                ...
                let _ = adi_api_proxy_core::db::usage::log_usage(
                    state_clone.db.pool(),
                    token.id,
                    token.user_id,
                    &request_id,
