...
// ============================================================================
// Server: Handler Wrappers & Router Tests
// ============================================================================

#[test]
fn test_server_generates_handler_wrappers() {
    let source = r#"
        @format("uuid")
        scalar uuid extends string;

        model User { id: uuid; name: string; }

        @route("/users")
        interface UserService {
            @get
            list(): { @statusCode statusCode: 200; @body body: User[]; } | ApiError;

            @get
            @route("/{id}")
            get(@path id: uuid): { @statusCode statusCode: 200; @body body: User; } | ApiError;

            @post
            create(@body body: User): { @statusCode statusCode: 201; @body body: User; } | ApiError;

            @delete
            @route("/{id}")
            delete(@path id: uuid): { @statusCode statusCode: 204; } | ApiError;
        }
    "#;
    ...
    // Handler wrappers should exist
    assert!(server.contains("async fn user_service_list<S: UserServiceHandler>"));
    assert!(server.contains("async fn user_service_get<S: UserServiceHandler>"));
    assert!(server.contains("async fn user_service_create<S: UserServiceHandler>"));
    assert!(server.contains("async fn user_service_delete<S: UserServiceHandler>"));

    // 204 should return StatusCode::NO_CONTENT
    assert!(server.contains("StatusCode::NO_CONTENT"));

    // 201 should return (StatusCode::CREATED, Json(...))
    assert!(server.contains("StatusCode::CREATED"));

    // Router factory
    assert!(server.contains("pub fn user_service_routes<S: UserServiceHandler>"));
    assert!(server.contains("create_router"));
}

#[test]
fn test_server_response_type_extraction() {
    let source = r#"
        ...
        interface ItemService {
            @get
            list(): { @statusCode statusCode: 200; @body body: Item[]; } | ApiError;
        }
    "#;
    ...
    // Trait should return the body type, not the raw union
    assert!(server.contains("-> Result<Vec<Item>, ApiError>"));
    // Handler should return Json
    assert!(server.contains("Json<Vec<Item>>"));
}

#[test]
fn test_server_spread_params_resolved_as_query() {
    let source = r#"
        model PaginationParams {
            @query page?: int32;
            @query limit?: int32;
        ...
        interface ItemService {
            @get
            list(...PaginationParams): { @statusCode statusCode: 200; @body body: string[]; } | ApiError;
        }
    "#;
    ...
    // Should generate a query struct
    assert!(server.contains("pub struct ItemServiceListQuery"));
    assert!(server.contains("pub page: Option<i32>"));
    assert!(server.contains("pub limit: Option<i32>"));

    // Trait should accept the query struct
    assert!(server.contains("query: ItemServiceListQuery"));

    // Handler should extract Query
    assert!(server.contains("Query(query): Query<ItemServiceListQuery>"));
}

#[test]
fn test_server_router_groups_methods_by_path() {
    let source = r#"
        @format("uuid")
        scalar uuid extends string;
        ...
        interface ItemService {
            ...
            @get @route("/{id}")
            get(@path id: uuid): Item;

            @patch @route("/{id}")
            update(@path id: uuid, @body body: Item): Item;

            @delete @route("/{id}")
            delete(@path id: uuid): Item;
        }
    "#;
    ...
    // Routes should use axum :id format, not {id}
    assert!(server.contains(":id"));
    assert!(!server.contains("\"{id}\""));

    // Methods on same path should be chained
    assert!(server.contains("get(item_service_get::<S>)"));
    assert!(server.contains("delete(item_service_delete::<S>)"));
}

#[test]
fn test_server_multiple_interfaces_combined_router() {
    let source = r#"
    ...
    // Both traits
    assert!(server.contains("pub trait UserServiceHandler"));
    assert!(server.contains("pub trait TaskServiceHandler"));

    // Both route factories
    assert!(server.contains("pub fn user_service_routes"));
    assert!(server.contains("pub fn task_service_routes"));

    // Combined router with both trait bounds
    assert!(server.contains("UserServiceHandler + TaskServiceHandler"));
    assert!(server.contains(".merge(user_service_routes())"));
    assert!(server.contains(".merge(task_service_routes())"));
}

#[test]
fn test_server_path_and_body_combined() {
    let source = r#"
        @format("uuid")
        scalar uuid extends string;

        model UpdateRequest { name: string; }
        model Item { id: uuid; name: string; }

        @route("/items")
        interface ItemService {
            @patch
            @route("/{id}")
            update(@path id: uuid, @body body: UpdateRequest): {
    ...
    // Handler should have both Path and Json extractors
    assert!(server.contains("Path(id): Path<Uuid>"));
    assert!(server.contains("Json(body): Json<UpdateRequest>"));
    // Trait should have both params
    assert!(server.contains("id: Uuid, body: UpdateRequest"));
}
