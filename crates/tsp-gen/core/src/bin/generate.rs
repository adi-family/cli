...
struct Cli {
    ...
    /// Axum path parameter style (colon = :id for 0.7, brace = {id} for 0.8+)
    #[arg(long, default_value = "colon")]
    path_style: String,

    /// State wrapper (arc = State<Arc<S>>, clone = State<S> where S: Clone)
    #[arg(long, default_value = "arc")]
    state_wrapper: String,

    /// Request context type extracted from requests and passed to trait methods (e.g. AuthUser)
    #[arg(long)]
    request_context: Option<String>,

    /// External types crate name (when using side=server with shared types)
    #[arg(long)]
    ...
    /// Serde rename strategy for protocol generation (e.g., "snake_case")
...
fn do_generate(cli: &Cli) -> Result<Vec<String>> {
    ...
    let state_wrapper = match cli.state_wrapper.as_str() {
        "clone" => StateWrapper::Clone,
        _ => StateWrapper::Arc,
    };
    let rust_config = RustServerConfig {
        path_style,
        state_wrapper,
        request_context: cli.request_context.clone(),
    };

    let mut generator = Generator::new(&combined, &output_dir, &cli.package)
        .with_rust_config(rust_config);
    ...
    if cli.side == Side::Protocol {
        let protocol_config = RustProtocolConfig {
            tag: cli.protocol_tag.clone(),
            rename: cli.protocol_rename.clone(),
            enum_name: cli.protocol_enum_name.clone(),
        };
        generator = generator.with_rust_protocol_config(protocol_config);
    }

    // Build AdiService config if needed
    if cli.side == Side::AdiService {
        let types_crate = cli
            .types_crate
            .clone()
        ...
        generator = generator.with_rust_adi_config(adi_config);
    }
