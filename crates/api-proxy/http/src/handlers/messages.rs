...
use api_proxy_core::{ApiError, ApiResult, ProviderType, ProxyRequest, RequestStatus};
...
pub async fn messages(
    ...
    let provider_type = match token.key_mode {
        api_proxy_core::KeyMode::Byok => {
            let key_id = token.upstream_key_id.ok_or_else(|| {
                ApiError::Internal("BYOK token missing upstream_key_id".to_string())
            })?;
            let key = api_proxy_core::db::keys::get_upstream_key_by_id(state.db.pool(), key_id)
                .await?;
            key.provider_type
        }
        api_proxy_core::KeyMode::Platform => token.platform_provider.ok_or_else(|| {
    ...
    match stream_result {
        Ok(upstream_stream) => {
            ...
            tokio::spawn(async move {
                ...
                let mut usage_info: Option<api_proxy_core::UsageInfo> = None;
                ...
                let _ = api_proxy_core::db::usage::log_usage(
                    state_clone.db.pool(),
                    token.id,
                    token.user_id,
                    &request_id,
