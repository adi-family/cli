    ...
    match result {
        ...
        Err(e) => {
            let (status, error_type) = match &e {
                api_proxy_core::providers::ProviderError::AuthenticationFailed => {
                    (StatusCode::UNAUTHORIZED, "authentication_failed")
                }
                api_proxy_core::providers::ProviderError::RateLimited => {
                    (StatusCode::TOO_MANY_REQUESTS, "rate_limited")
                }
                api_proxy_core::providers::ProviderError::Timeout => {
    ...
    match stream_result {
        Ok(upstream_stream) => {
            ...
            tokio::spawn(async move {
                ...
                let log_result = api_proxy_core::db::usage::log_usage(
                    state_clone.db.pool(),
                    token.id,
                    token.user_id,
                    &request_id,
