...
pub fn keys_service_routes<S: KeysServiceHandler>() -> Router<Arc<S>> {
    Router::new()
        .route("/api/proxy/keys", post(keys_service_create_key::<S>).get(keys_service_list_keys::<S>))
        .route("/api/proxy/keys/:id", get(keys_service_get_key::<S>).patch(keys_service_update_key::<S>).delete(keys_service_delete_key::<S>))
        .route("/api/proxy/keys/:id/verify", post(keys_service_verify_key::<S>))
}
...
pub fn platform_keys_service_routes<S: PlatformKeysServiceHandler>() -> Router<Arc<S>> {
    Router::new()
        .route("/api/proxy/platform-keys", get(platform_keys_service_list_platform_keys::<S>).post(platform_keys_service_upsert_platform_key::<S>))
        .route("/api/proxy/platform-keys/:id", patch(platform_keys_service_update_platform_key::<S>).delete(platform_keys_service_delete_platform_key::<S>))
        .route("/api/proxy/platform-keys/:providerType/verify", post(platform_keys_service_verify_platform_key::<S>))
}
...
pub fn tokens_service_routes<S: TokensServiceHandler>() -> Router<Arc<S>> {
    Router::new()
        .route("/api/proxy/tokens", post(tokens_service_create_token::<S>).get(tokens_service_list_tokens::<S>))
        .route("/api/proxy/tokens/:id", get(tokens_service_get_token::<S>).patch(tokens_service_update_token::<S>).delete(tokens_service_delete_token::<S>))
        .route("/api/proxy/tokens/:id/rotate", post(tokens_service_rotate_token::<S>))
}
...
pub fn providers_service_routes<S: ProvidersServiceHandler>() -> Router<Arc<S>> {
    Router::new()
        .route("/api/proxy/providers", get(providers_service_list_providers::<S>))
}
...
pub fn usage_service_routes<S: UsageServiceHandler>() -> Router<Arc<S>> {
    Router::new()
        .route("/api/proxy/usage", get(usage_service_query_usage::<S>))
        .route("/api/proxy/usage/summary", get(usage_service_usage_summary::<S>))
        .route("/api/proxy/usage/export", get(usage_service_export_usage::<S>))
}
