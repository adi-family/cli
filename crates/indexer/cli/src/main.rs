...
fn init_project(project_path: &PathBuf) -> Result<()> {
    ...
    let config = indexer_core::Config::default();
...
async fn index_project(project_path: &PathBuf, _force: bool) -> Result<()> {
    ...
    let adi = indexer_core::Adi::open(project_path).await?;
...
fn print_symbol(symbol: &indexer_core::SymbolNode, indent: usize) {
...
async fn show_symbol(project_path: &PathBuf, id: i64) -> Result<()> {
    let adi = indexer_core::Adi::open(project_path).await?;
    let symbol = adi.get_symbol(indexer_core::SymbolId(id))?;
...
fn config_cmd(project_path: &PathBuf, key: Option<String>, value: Option<String>) -> Result<()> {
    let config = indexer_core::Config::load(project_path)?;
...
async fn find_usages(project_path: &PathBuf, query: &str, limit: usize) -> Result<()> {
    ...
    if let Ok(id) = query.parse::<i64>() {
        ...
        let usage = adi.get_symbol_usage(indexer_core::SymbolId(id))?;
...
fn print_symbol_usage(usage: &indexer_core::SymbolUsage, adi: &indexer_core::Adi) {
...
async fn show_callgraph(
    ...
    if show_callers {
        let callers = adi.get_callers(indexer_core::SymbolId(id))?;
        println!("\nCallers ({}):", callers.len());
        if callers.is_empty() {
            println!("  (none)");
        } else {
    ...
    if show_callees {
        let callees = adi.get_callees(indexer_core::SymbolId(id))?;
        println!("\nCallees ({}):", callees.len());
        if callees.is_empty() {
            println!("  (none)");
        } else {
...
async fn show_refs(project_path: &PathBuf, id: i64, direction: &str) -> Result<()> {
    ...
    if show_to {
        let refs_to = adi.get_references_to(indexer_core::SymbolId(id))?;
    ...
    if show_from {
        let refs_from = adi.get_references_from(indexer_core::SymbolId(id))?;
...
async fn find_dead_code(
    ...
    let storage = indexer_core::storage::sqlite::SqliteStorage::open(
