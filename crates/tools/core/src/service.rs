...
impl ToolsService {
    ...
    pub async fn call_tool(&self, name: &str, arguments: JsonValue) -> Result<ToolResult, String> {
        ...
        provider.call_tool(name, arguments).await
    }

    /// List all registered providers with their tool counts.
    pub fn list_providers(&self) -> Vec<JsonValue> {
        self.providers
            .iter()
            .map(|p| {
                json!({
