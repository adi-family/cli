//! Russian translation plugin for ADI Workflow (v3)
//!
...
pub struct RussianTranslationPlugin {
    metadata: TranslationMetadata,
}

impl RussianTranslationPlugin {
    pub fn new() -> Self {
        Self {
            metadata: TranslationMetadata {
                plugin_id: "adi.workflow".to_string(),
                language: "ru-RU".to_string(),
                language_name: "Русский".to_string(),
...
#[async_trait]
impl Plugin for RussianTranslationPlugin {
    fn metadata(&self) -> PluginMetadata {
        PluginMetadata {
            id: "adi.workflow.ru-RU".to_string(),
            name: "ADI Workflow - Русский".to_string(),
            version: "3.0.0".to_string(),
            plugin_type: PluginType::Extension,
            author: Some("ADI Team".to_string()),
            description: Some("Русские переводы для ADI Workflow".to_string()),
            category: None,
        }
    }
...
impl Default for RussianTranslationPlugin {
    fn default() -> Self {
        Self::new()
    }
}
...
pub fn plugin_create() -> Box<dyn Plugin> {
    Box::new(RussianTranslationPlugin::new())
}
