//! Japanese translation plugin for ADI Workflow (v3)
//!
...
pub struct JapaneseTranslationPlugin {
    metadata: TranslationMetadata,
}

impl JapaneseTranslationPlugin {
    pub fn new() -> Self {
        Self {
            metadata: TranslationMetadata {
                plugin_id: "adi.workflow".to_string(),
                language: "ja-JP".to_string(),
                language_name: "日本語".to_string(),
...
#[async_trait]
impl Plugin for JapaneseTranslationPlugin {
    fn metadata(&self) -> PluginMetadata {
        PluginMetadata {
            id: "adi.workflow.ja-JP".to_string(),
            name: "ADI Workflow - 日本語".to_string(),
            version: "3.0.0".to_string(),
            plugin_type: PluginType::Extension,
            author: Some("ADI Team".to_string()),
            description: Some("ADI Workflow の日本語翻訳".to_string()),
            category: None,
        }
    }
...
impl Default for JapaneseTranslationPlugin {
    fn default() -> Self {
        Self::new()
    }
}
...
pub fn plugin_create() -> Box<dyn Plugin> {
    Box::new(JapaneseTranslationPlugin::new())
}
