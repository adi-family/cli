//! Ukrainian translation plugin for ADI Workflow (v3)
//!
...
pub struct UkrainianTranslationPlugin {
    metadata: TranslationMetadata,
}

impl UkrainianTranslationPlugin {
    pub fn new() -> Self {
        Self {
            metadata: TranslationMetadata {
                plugin_id: "adi.workflow".to_string(),
                language: "uk-UA".to_string(),
                language_name: "Українська".to_string(),
...
#[async_trait]
impl Plugin for UkrainianTranslationPlugin {
    fn metadata(&self) -> PluginMetadata {
        PluginMetadata {
            id: "adi.workflow.uk-UA".to_string(),
            name: "ADI Workflow - Українська".to_string(),
            version: "3.0.0".to_string(),
            plugin_type: PluginType::Extension,
            author: Some("ADI Team".to_string()),
            description: Some("Українські переклади для ADI Workflow".to_string()),
            category: None,
        }
    }
...
impl Default for UkrainianTranslationPlugin {
    fn default() -> Self {
        Self::new()
    }
}
...
pub fn plugin_create() -> Box<dyn Plugin> {
    Box::new(UkrainianTranslationPlugin::new())
}
