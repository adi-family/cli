...
#[derive(Parser)]
#[command(name = "adi-indexer-cli")]
#[command(about = "ADI Indexer CLI - AI-powered code indexer and search")]
#[command(version)]
struct Cli {
...
enum Commands {
    ...
    /// Find dead code (unused symbols)
    DeadCode {
        /// Analysis mode: strict, library, or application
...
fn init_project(project_path: &PathBuf) -> Result<()> {
    ...
    let config = adi_indexer_core::Config::default();
    ...
    println!("Run 'adi-indexer-cli index' to index the project");
...
async fn index_project(project_path: &PathBuf, _force: bool) -> Result<()> {
    ...
    let adi = adi_indexer_core::Adi::open(project_path).await?;
...
fn print_symbol(symbol: &adi_indexer_core::SymbolNode, indent: usize) {
...
async fn show_symbol(project_path: &PathBuf, id: i64) -> Result<()> {
    let adi = adi_indexer_core::Adi::open(project_path).await?;
    let symbol = adi.get_symbol(adi_indexer_core::SymbolId(id))?;
...
fn config_cmd(project_path: &PathBuf, key: Option<String>, value: Option<String>) -> Result<()> {
    let config = adi_indexer_core::Config::load(project_path)?;
...
async fn find_usages(project_path: &PathBuf, query: &str, limit: usize) -> Result<()> {
    ...
    if let Ok(id) = query.parse::<i64>() {
        ...
        let usage = adi.get_symbol_usage(adi_indexer_core::SymbolId(id))?;
    ...
    if symbols.is_empty() {
        ...
        println!("\nUse 'adi-indexer-cli usages <id>' to see detailed usage for a specific symbol");
        return Ok(());
    }
...
fn print_symbol_usage(usage: &adi_indexer_core::SymbolUsage, adi: &adi_indexer_core::Adi) {
...
async fn show_callgraph(
    ...
    if show_callers {
        let callers = adi.get_callers(adi_indexer_core::SymbolId(id))?;
        println!("\nCallers ({}):", callers.len());
        if callers.is_empty() {
            println!("  (none)");
        } else {
    ...
    if show_callees {
        let callees = adi.get_callees(adi_indexer_core::SymbolId(id))?;
        println!("\nCallees ({}):", callees.len());
        if callees.is_empty() {
            println!("  (none)");
        } else {
...
async fn show_refs(project_path: &PathBuf, id: i64, direction: &str) -> Result<()> {
    ...
    if show_to {
        let refs_to = adi.get_references_to(adi_indexer_core::SymbolId(id))?;
    ...
    if show_from {
        let refs_from = adi.get_references_from(adi_indexer_core::SymbolId(id))?;
...
async fn find_dead_code(
    ...
    let analysis_mode = match mode {
        ...
        _ => {
            println!("Invalid mode: {}. Using 'strict'", mode);
            AnalysisMode::Strict
        }
    };
    ...
    let storage = adi_indexer_core::storage::sqlite::SqliteStorage::open(
        &project_path.join(".adi/tree/index.sqlite"),
    )?;
    let analyzer = DeadCodeAnalyzer::new(std::sync::Arc::new(storage), config);

    println!("Analyzing dead code...");
