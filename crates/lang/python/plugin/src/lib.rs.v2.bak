...
const SERVICE_ID: &str = "adi.indexer.lang.python";
...
extern "C" fn plugin_info() -> PluginInfo {
    ...
    .with_description("Python language parsing and analysis for ADI indexer")
...
extern "C" fn plugin_init(ctx: *mut PluginContext) -> i32 {
    unsafe {
        ...
        let descriptor =
            ServiceDescriptor::new(SERVICE_ID, ServiceVersion::new(1, 0, 0), "adi.lang.python")
                .with_description("Python language analyzer for code indexing");
        ...
        if let Err(code) = host.register_svc(descriptor, handle) {
            host.error(&format!(
                "Failed to register Python analyzer service: {}",
                code
            ));
            return code;
        }

        host.info("Python language plugin initialized");
    }
    0
}
