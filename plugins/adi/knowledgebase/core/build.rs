...
fn main() {
    ...
    let kb_dir = PathBuf::from(&out_dir).join("knowledgebase_adi");
    let adi_config = RustAdiServiceConfig {
        types_crate: "crate".into(),
        cocoon_crate: "lib_adi_service".into(),
        service_name: "Knowledgebase".into(),
        ..Default::default()
    };

    Generator::new(&file, &kb_dir, "knowledgebase")
        .with_rust_adi_config(adi_config)
        .generate(Language::Rust, Side::AdiService)
        .expect("knowledgebase adi codegen failed");

    let adi_src = kb_dir.join("src/adi_service.rs");
    if adi_src.exists() {
        let content = std::fs::read_to_string(&adi_src).unwrap();
        std::fs::write(format!("{out_dir}/knowledgebase_adi_service.rs"), content).unwrap();
    }
}
