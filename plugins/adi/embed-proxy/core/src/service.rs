include!(concat!(env!("OUT_DIR"), "/embed_proxy_adi_service.rs"));
...
pub struct EmbedProxyService {
    db: PgPool,
    secrets: SecretManager,
}

impl EmbedProxyService {
...
impl EmbedProxyServiceHandler for EmbedProxyService {
    ...
    async fn upsert_platform_key(
        ...
        let encrypted = self
            ...
            .map_err(|e| AdiServiceError::internal(e.to_string()))?;
        let k =
            db::upsert_platform_key(&self.db, provider_type, &encrypted, base_url.as_deref())
                .await
                .map_err(Self::map_error)?;
    ...
    async fn query_usage(
        ...
        let logs =
            db::query_usage(&self.db, user_id, proxy_token_id, from_dt, to_dt, lim, off)
                .await
                .map_err(Self::map_error)?;

        let summary =
            db::get_usage_summary(&self.db, user_id, proxy_token_id, from_dt, to_dt)
                .await
                .map_err(Self::map_error)?;
