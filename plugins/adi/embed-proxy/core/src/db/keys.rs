...
pub async fn create_upstream_key(
    ...
    let row = sqlx::query(
        r#"
        INSERT INTO embed_upstream_api_keys (user_id, name, provider_type, api_key_encrypted, base_url)
        VALUES ($1, $2, $3, $4, $5)
    ...
    .map_err(|e| match &e {
        sqlx::Error::Database(db_err)
            if db_err.constraint() == Some("embed_upstream_api_keys_user_id_name_key") =>
        {
...
pub async fn update_upstream_key(
    ...
    let row = sqlx::query(
        r#"
        UPDATE embed_upstream_api_keys
        SET
...
pub async fn delete_upstream_key(pool: &PgPool, id: Uuid, user_id: Uuid) -> ApiResult<()> {
    let result =
        sqlx::query("DELETE FROM embed_upstream_api_keys WHERE id = $1 AND user_id = $2")
            .bind(id)
            .bind(user_id)
            .execute(pool)
            .await?;
