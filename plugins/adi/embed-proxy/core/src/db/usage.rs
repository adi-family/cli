...
use crate::types::{EmbedUsageLog, KeyMode, ProviderType, RequestStatus};
...
) -> ApiResult<EmbedUsageLog> {
    let row = sqlx::query(
        r#"
        INSERT INTO embed_proxy_usage_log (
            proxy_token_id, user_id, request_id, upstream_request_id,
            requested_model, actual_model, provider_type, key_mode,
            input_tokens, total_tokens, dimensions, input_count,
            reported_cost_usd, endpoint, latency_ms,
...
pub async fn query_usage(
    ...
    limit: i64,
    offset: i64,
) -> ApiResult<Vec<EmbedUsageLog>> {
    let rows = sqlx::query(
        r#"
        SELECT *
        FROM embed_proxy_usage_log
        WHERE user_id = $1
...
fn row_to_usage_log(row: &sqlx::postgres::PgRow) -> EmbedUsageLog {
