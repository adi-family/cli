...
use embed_proxy_core::{Config, Database, SecretManager};
...
pub fn run_server() -> anyhow::Result<()> {
    ...
    rt.block_on(async {
        ...
        tracing_subscriber::registry()
            .with(
                tracing_subscriber::EnvFilter::try_from_default_env()
                    .unwrap_or_else(|_| "embed_proxy_http=debug,tower_http=debug".into()),
            )
        ...
        tracing::info!("Starting Embed Proxy HTTP server on {}", addr);
...
fn router(state: AppState) -> Router {
    ...
    Router::new()
        ...
        .nest("/api/embed-proxy", management)
        .nest("/v1", proxy)
