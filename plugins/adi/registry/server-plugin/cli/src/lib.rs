...
pub struct CliRegistryPlugin;

impl CliRegistryPlugin {
    pub fn new() -> Self {
        Self
    }
}
...
impl Plugin for CliRegistryPlugin {
    fn metadata(&self) -> PluginMetadata {
        PluginMetadata {
            ...
            description: Some("CLI plugin registry server".to_string()),
            category: None,
        }
    }
...
#[async_trait]
impl CliCommands for CliRegistryPlugin {
        ...
        match subcommand {
            "start" => {
                ...
                if let Err(e) = server::run_server(port) {
                    return Ok(CliResult::error(format!("CLI registry server failed: {e}")));
                }
                Ok(CliResult::success("CLI registry server stopped"))
            }
            _ => Ok(CliResult::error(format!(
                "Unknown command: {subcommand}\nUsage: adi cli-registry-server start [--port PORT]"
            ))),
        }
    }
}
