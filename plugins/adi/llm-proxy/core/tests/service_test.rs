//! Integration tests for LlmProxyService handler methods.
//!
...
use llm_proxy_core::crypto::SecretManager;
use llm_proxy_core::models::*;
use llm_proxy_core::service::{LlmProxyService, LlmProxyServiceHandler};
use sqlx::PgPool;
use uuid::Uuid;
...
fn svc(pool: PgPool) -> LlmProxyService {
    LlmProxyService::new(pool, test_secrets())
}
...
async fn create_key_fail_duplicate_name(pool: PgPool) {
    ...
    let err = s
        ...
        .await
        .unwrap_err();
    // DB unique constraint violation maps to internal/database error
    assert!(
...
async fn upsert_platform_key_success_create(pool: PgPool) {
    ...
    let key = s
        ...
        .await
        .unwrap();
    assert_eq!(key.provider_type, ProviderType::Anthropic);
    assert!(key.is_active);
}
...
async fn upsert_platform_key_success_update(pool: PgPool) {
    ...
    // Same provider type → same row updated
...
#[sqlx::test(migrations = "../migrations")]
async fn update_platform_key_fail_not_found_random_id(pool: PgPool) {
    let s = svc(pool);
    let err = s
...
async fn delete_platform_key_success(pool: PgPool) {
    ...
    let ctx = authed_ctx();
    let key = s
        .upsert_platform_key(&ctx, ProviderType::OpenRouter, "sk-or".into(), None)
        .await
        .unwrap();
...
async fn create_token_success_platform(pool: PgPool) {
    ...
    assert_eq!(resp.token.allowed_models, Some(vec!["gpt-4".into()]));
...
async fn create_token_fail_byok_without_key(pool: PgPool) {
    let s = svc(pool);
    // DB CHECK constraint: byok requires upstream_key_id
    let err = s
        .create_token(
            &authed_ctx(),
            "bad".into(),
...
async fn update_token_success(pool: PgPool) {
    ...
    assert_eq!(updated.allowed_models, Some(vec!["gpt-4o".into()]));
...
async fn list_providers_success_empty(pool: PgPool) {
    ...
    // Returns all provider types, none available
