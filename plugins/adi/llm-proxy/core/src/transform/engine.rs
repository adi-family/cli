...
mod tests {
    ...
    fn test_eval_is_disabled() {
        ...
        let headers = http::HeaderMap::new();
        let body = serde_json::json!({});
        let result = engine.transform_request_body(r#"eval("1+1")"#, "POST", "/test", &headers, body);
        assert!(result.is_err());
    }
    ...
    fn test_builtin_functions_available() {
        ...
        // timestamp() should return a positive integer
        let script = r#"body.ts = timestamp();"#;
        let result = engine.transform_request_body(script, "POST", "/test", &headers, body).unwrap();
    ...
    fn test_request_model_override() {
        let engine = TransformEngine::new();
        let headers = http::HeaderMap::new();
        let body = serde_json::json!({"model": "gpt-3.5-turbo", "messages": []});

        // Script overrides model in body
        let script = r#"body.model = "gpt-4";"#;
        let result = engine.transform_request_body(script, "POST", "/v1/chat/completions", &headers, body).unwrap();
        assert_eq!(result["model"], "gpt-4");
    }

    #[test]
    fn test_response_transform_modifies_body() {
        let engine = TransformEngine::new();
        let headers = http::HeaderMap::new();
        let body = serde_json::json!({"choices": [{"text": "hello"}]});

        let script = r#"body.filtered = true;"#;
        let result = engine.transform_response_body(script, 200, &headers, body, None, None).unwrap();
        assert_eq!(result["filtered"], true);
        assert_eq!(result["choices"][0]["text"], "hello");
    }

    #[test]
    fn test_response_transform_has_token_info() {
        let engine = TransformEngine::new();
        let headers = http::HeaderMap::new();
        let body = serde_json::json!({});

        let script = r#"body.total = input_tokens + output_tokens;"#;
        let result = engine.transform_response_body(script, 200, &headers, body, Some(100), Some(50)).unwrap();
        assert_eq!(result["total"], 150);
    }

    #[test]
    fn test_request_context_variables_accessible() {
        let engine = TransformEngine::new();
        let headers = http::HeaderMap::new();
        let body = serde_json::json!({});

        let script = r#"
            body.captured_method = method;
            body.captured_path = path;
        "#;
        let result = engine.transform_request_body(script, "POST", "/v1/chat/completions", &headers, body).unwrap();
        assert_eq!(result["captured_method"], "POST");
        assert_eq!(result["captured_path"], "/v1/chat/completions");
    }
}
