...
mod tests {
    ...
    fn test_json_null_roundtrip() {
        let dynamic = json_to_dynamic(&serde_json::Value::Null);
        assert_eq!(dynamic_to_json(&dynamic).unwrap(), serde_json::Value::Null);
    }

    #[test]
    fn test_json_bool_roundtrip() {
        let dynamic = json_to_dynamic(&serde_json::json!(true));
        assert_eq!(dynamic_to_json(&dynamic).unwrap(), serde_json::json!(true));
    }

    #[test]
    fn test_json_string_roundtrip() {
        let dynamic = json_to_dynamic(&serde_json::json!("hello world"));
        assert_eq!(dynamic_to_json(&dynamic).unwrap(), serde_json::json!("hello world"));
    }

    #[test]
    fn test_json_nested_array_roundtrip() {
        let json = serde_json::json!([[1, 2], [3, 4]]);
        let dynamic = json_to_dynamic(&json);
        assert_eq!(dynamic_to_json(&dynamic).unwrap(), json);
    }

    #[test]
    fn test_request_context_extracts_model() {
        let headers = http::HeaderMap::new();
        let body = serde_json::json!({"model": "claude-3-opus", "messages": []});
        let ctx = RequestContext::new("POST", "/v1/messages", &headers, body);
        assert_eq!(ctx.model, Some("claude-3-opus".to_string()));
        assert_eq!(ctx.method, "POST");
        assert_eq!(ctx.path, "/v1/messages");
    }

    #[test]
    fn test_request_context_no_model() {
        let headers = http::HeaderMap::new();
        let body = serde_json::json!({"messages": []});
        let ctx = RequestContext::new("POST", "/v1/embeddings", &headers, body);
        assert_eq!(ctx.model, None);
    }

    #[test]
    fn test_request_context_headers_converted() {
        let mut headers = http::HeaderMap::new();
        headers.insert("x-custom", "value123".parse().unwrap());
        let body = serde_json::json!({});
        let ctx = RequestContext::new("GET", "/test", &headers, body);
        let header_val = ctx.headers.get("x-custom").unwrap();
        assert_eq!(header_val.clone().into_string().unwrap(), "value123");
    }

    #[test]
    fn test_response_context_extracts_model_and_tokens() {
        let headers = http::HeaderMap::new();
        let body = serde_json::json!({"model": "gpt-4", "choices": []});
        let ctx = ResponseContext::new(200, &headers, body, Some(10), Some(20));
        assert_eq!(ctx.model, Some("gpt-4".to_string()));
        assert_eq!(ctx.status_code, 200);
        assert_eq!(ctx.input_tokens, Some(10));
        assert_eq!(ctx.output_tokens, Some(20));
    }

    #[test]
    fn test_response_context_to_json_body() {
        let headers = http::HeaderMap::new();
        let body = serde_json::json!({"result": "ok"});
        let ctx = ResponseContext::new(200, &headers, body.clone(), None, None);
        assert_eq!(ctx.to_json_body().unwrap(), body);
    }
}
