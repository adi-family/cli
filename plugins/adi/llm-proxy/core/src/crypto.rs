...
mod tests {
    ...
    fn test_different_keys_cannot_decrypt() {
        let manager_a = SecretManager::new([0u8; 32]);
        let manager_b = SecretManager::new([1u8; 32]);

        let encrypted = manager_a.encrypt("secret").unwrap();
        assert!(manager_b.decrypt(&encrypted).is_err());
    }

    #[test]
    fn test_is_encrypted() {
        assert!(SecretManager::is_encrypted("ENC:abc"));
        assert!(!SecretManager::is_encrypted("sk-test-key"));
    ...
    fn test_encrypt_produces_different_ciphertexts() {
        ...
        let e1 = manager.encrypt("same").unwrap();
        let e2 = manager.encrypt("same").unwrap();
        assert_ne!(e1, e2); // random nonce
        assert_eq!(manager.decrypt(&e1).unwrap(), "same");
        assert_eq!(manager.decrypt(&e2).unwrap(), "same");
    }

    #[test]
    fn test_from_hex_wrong_length() {
        assert!(SecretManager::from_hex("0011").is_err());
        assert!(SecretManager::from_hex("not-hex").is_err());
    }

    #[test]
    fn test_encrypted_data_too_short() {
        let manager = SecretManager::new([0u8; 32]);
        assert!(manager.decrypt("ENC:AQID").is_err());
    }

    #[test]
    fn test_encrypt_empty_and_long() {
        let manager = SecretManager::new([0u8; 32]);

        let enc = manager.encrypt("").unwrap();
        assert_eq!(manager.decrypt(&enc).unwrap(), "");

        let long = "a".repeat(10_000);
        let enc = manager.encrypt(&long).unwrap();
        assert_eq!(manager.decrypt(&enc).unwrap(), long);
    }
}
