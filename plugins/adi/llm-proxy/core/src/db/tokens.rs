...
mod tests {
    ...
    fn test_generate_token_format() {
        let (raw, prefix, hash) = generate_token();
        assert!(raw.starts_with("adi_pk_"));
        assert!(prefix.ends_with("..."));
        assert_eq!(prefix.len(), 15); // "adi_pk_XXXXX..."
        assert_eq!(hash.len(), 64); // SHA-256 hex
    }

    #[test]
    fn test_generate_token_unique() {
        let (raw1, _, hash1) = generate_token();
        let (raw2, _, hash2) = generate_token();
        assert_ne!(raw1, raw2);
    ...
    fn test_hash_token_deterministic() {
        let token = "adi_pk_test12345";
        assert_eq!(hash_token(token), hash_token(token));
    }

    #[test]
    fn test_hash_token_matches_generate() {
        let (raw, _, hash) = generate_token();
        assert_eq!(hash_token(&raw), hash);
    }

    #[test]
    fn test_hash_token_different_inputs() {
        assert_ne!(hash_token("adi_pk_aaa"), hash_token("adi_pk_bbb"));
    }
}
