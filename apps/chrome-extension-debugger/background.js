...
async function initializeExtension() {
  ...
  const stored = await chrome.storage.local.get(['browserId', 'signalingUrl', 'responseBodyMaxSize']);
...
function handleGetNetwork(message) {
  ...
  if (filters) {
    ...
    if (filters.status_min !== undefined) {
      requests = requests.filter(r => r.status !== undefined && r.status >= filters.status_min);
    }
    
    if (filters.status_max !== undefined) {
      requests = requests.filter(r => r.status !== undefined && r.status <= filters.status_max);
    }
...
chrome.webRequest.onHeadersReceived.addListener(
  (details) => {
    ...
    if (debugTokenHeader && debugTokenHeader.value) {
      ...
      console.log('[ADI Debug] Found debug token on tab', details.tabId, 'URL:', details.url);
...
async function startDebugging(tabId, token, url) {
  ...
  try {
    ...
    console.error('[ADI Debug] Failed to attach debugger to tab', tabId, ':', error);
  }
}
...
async function stopDebugging(tabId) {
  ...
  try {
    ...
    console.log('[ADI Debug] Could not detach debugger (tab may be closed):', error.message);
  }
...
chrome.debugger.onDetach.addListener((source, reason) => {
  ...
  if (debugTabs.has(tabId)) {
    const tabData = debugTabs.get(tabId);
    console.log('[ADI Debug] Debugger detached from tab', tabId, '- reason:', reason);
