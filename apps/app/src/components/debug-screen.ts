...
export class AppDebugScreen extends LitElement {
  ...
  @state() private serverStates: Map<
    string,
    {
      wsState: WsState;
      cocoons: CocoonInfo[];
      authError: { reason: string; kind?: string; domain?: string } | null;
  ...
  async #checkRegistry(registry: HttpPluginRegistry): Promise<void> {
    ...
    this.registries = this.registries.map((r) =>
      r.registry === registry
        ? { ...r, checking: false, health, checkedAt }
        : r,
    );
  }
  ...
  #checkAllRegistries(): void {
    for (const { registry } of this.registries)
      void this.#checkRegistry(registry);
  }
    ...
    this.signalingUnsubs.push(
      ...
      bus.on(
        ...
        ({ url, reason, authKind, authDomain }) => {
          ...
          next.set(url, {
            ...entry,
            authError: { reason, kind: authKind, domain: authDomain },
          });
          this.serverStates = next;
        },
        "debug-screen",
  ...
  #renderTab(id: Tab, label: string, count?: number) {
    return html`
      ...
      >
        ...
        ${count !== undefined
          ? html`<span
              class="ml-1.5 text-xs bg-surface-alt px-1.5 py-0.5 rounded-full"
              >${count}</span
            >`
          : nothing}
      </button>
  ...
  #statusBadge(id: string) {
    ...
    if (loaded.includes(id))
      return html`<span
        class="text-[10px] font-medium text-green-400 bg-green-400/10 px-1.5 py-0.5 rounded"
        >loaded</span
      >`;
    if (failed.includes(id))
      return html`<span
        class="text-[10px] font-medium text-red-400 bg-red-400/10 px-1.5 py-0.5 rounded"
        >failed</span
      >`;
    if (timedOut.includes(id))
      return html`<span
        class="text-[10px] font-medium text-yellow-400 bg-yellow-400/10 px-1.5 py-0.5 rounded"
        >timed out</span
      >`;
    return nothing;
  }
  ...
  #typeBadges(types: string[] | undefined) {
    ...
    return types.map(
      (t) => html`
        <span
          class="text-[10px] font-medium text-accent bg-accent/10 px-1.5 py-0.5 rounded border border-accent/20"
          >${t}</span
        >
      `,
    );
    ...
    if (plugins.length === 0) {
      return html`<div class="p-6">
        <p class="text-text-muted text-sm">
          No registry plugins discovered yet.
        </p>
      </div>`;
    }
    ...
    return html`
      <div class="p-6 space-y-0_25">
        ${this.pluginsDirty
          ? html`
              <div
                class="flex items-center justify-between px-3 py-2 mb-2 bg-yellow-400/10 border border-yellow-400/30 rounded text-yellow-400 text-xs"
              >
        ...
        ${plugins.map((p) => {
          ...
          return html`
            <div
              class="flex items-center gap-3 px-3 py-2.5 bg-surface-alt rounded border border-border"
            >
              ${isWeb
                ? html`
                    <input
              ...
              <code class="text-sm font-mono text-text flex-1 truncate"
                >${p.id}</code
              >
              <span class="text-xs text-text-muted font-mono shrink-0"
                >v${p.installedVersion}</span
              >
              ...
              <table class="w-full text-sm border-collapse">
                <thead>
                  <tr class="text-left border-b border-border">
                    <th
                      class="pb-2 pr-4 text-xs font-semibold text-text-muted uppercase tracking-wider"
                    >
                      Path
                    </th>
                    <th
                    ...
                    <th
                      class="pb-2 text-xs font-semibold text-text-muted uppercase tracking-wider"
                    >
                      Label
                    </th>
                  </tr>
                ...
                <tbody>
                  ${this.routes.map(
                    (r) => html`
                      <tr
                        class="border-b border-border/50 hover:bg-surface-alt transition-colors"
                      >
                    ...
                    (c) => html`
                      ...
                      >
                        ...
                        <td class="py-2.5">
                          ${c.shortcut
                            ? html`<kbd
                                class="text-xs bg-surface-alt border border-border rounded px-1.5 py-0.5 font-mono"
                                >${c.shortcut}</kbd
                              >`
  ...
  #renderConnections() {
    ...
    return html`
      <div class="p-6">
        ${connections.size === 0
          ? html`<p class="text-text-muted text-sm">
          ...
          : html`
              <div class="space-y-1">
                ${[...connections.values()].map(
                  (conn) => html`
                    <div
                      class="border border-border rounded-lg p-4 bg-surface-alt"
                    >
                      <div class="flex items-center gap-2 mb-3">
                        <span class="w-2 h-2 rounded-full bg-green-400"></span>
                        <code class="text-sm font-mono font-bold text-text"
                          >${conn.id}</code
                        >
                      </div>
                      <div class="flex flex-wrap gap-2">
                        ${conn.services.map(
                          (s) => html`
                            <span
                              class="text-xs bg-surface border border-border rounded px-2 py-1 font-mono text-text-muted"
                              >${s}</span
                            >
                          `,
                        )}
      ...
      <div class="p-6 space-y-6">
        ...
        <div class="flex gap-2">
          ...
          <select
            ...
            @change=${(e: Event) => {
              this.spawnCocoonServer = (e.target as HTMLSelectElement).value;
            }}
          >
            ${serverUrls.length === 0
        ...
        ${[...this.serverStates.entries()]
          ...
          .map(
            ([url, s]) => html`
              <div
                class="px-3 py-2 bg-red-400/10 border border-red-400/30 rounded text-red-400 text-xs space-y-1"
              >
                <div class="flex items-center gap-2">
                  ...
                  >
                    <path
                      ...
                      stroke-width="2"
                      d="M12 15v.01M12 9v3m0 8a9 9 0 100-18 9 9 0 000 18z"
                    />
                  </svg>
                  <span class="flex-1"
                    >Authentication required for
                    <code class="font-mono">${url}</code>:
                    ${s.authError!.reason}</span
                  >
                </div>
                ${s.authError!.kind || s.authError!.domain
                  ? html`
                      <div class="flex items-center gap-3 ml-6 text-text-muted">
                        ${s.authError!.kind
                          ? html`<span
                              >Auth:
                              <code class="text-text font-mono"
                                >${s.authError!.kind}</code
                              ></span
                            >`
                          : nothing}
                        ${s.authError!.domain
                          ? html`<span
                              >Domain:
                              <a
                                href="${s.authError!.domain}"
                                target="_blank"
                                rel="noopener"
                                class="text-accent hover:underline font-mono"
                                >${s.authError!.domain}</a
                              ></span
                            >`
                          : nothing}
                      </div>
        ...
        <!-- Spawn error banner -->
        ${this.spawnStatus.error
          ? html`
              <div
                class="flex items-center justify-between px-3 py-2 bg-red-400/10 border border-red-400/30 rounded text-red-400 text-xs"
              >
                ...
                <button
                  ...
                  @click=${() => {
                    this.spawnStatus = { spawning: false, error: null };
                  }}
                >
                  Dismiss
                </button>
        ...
        <!-- Connection info per server -->
        ${[...this.serverStates.entries()]
          .filter(([, s]) => s.connectionInfo)
          .map(([url, s]) => {
            const info = s.connectionInfo!;
            return html`
              <div
                class="px-3 py-2 bg-surface-alt border border-border/50 rounded text-xs space-y-2"
              >
                ...
                ${info.hives.length > 0
                  ? html`
                            ...
                            <div class="flex items-center gap-2">
                              ...
                              <div class="flex flex-wrap gap-1">
                                ${h.cocoon_kinds.map(
                                  (k) => html`
                                    <span
                                      class="text-[10px] bg-surface border border-border rounded px-1.5 py-0.5 font-mono text-accent"
                                      >${k}</span
                                    >
                                  `,
                                )}
                    ...
                    `
                  : html`<p class="text-text-muted">No hives connected</p>`}
              </div>
            `;
          })}
        ...
        ${allCocoons.length === 0
          ? html`<p class="text-text-muted text-sm">
              No cocoons discovered. Add a server and spawn a cocoon, or switch
              to the Signaling tab to list cocoons.
            </p>`
          : html`
              <div class="space-y-1">
                  ...
                  return html`
                    ...
                    >
                      ...
                      <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2">
                          <code class="text-xs font-mono text-text truncate"
                            >${c.name ?? c.device_id.slice(0, 12)}</code
                          >
                          <span
                      ...
                      <div class="flex items-center gap-1 shrink-0">
                        ...
                        >
                        ${session
                          ? this.#rtcStateBadge(session.state)
                          : nothing}
                        ${isConnected
                          ? html`<span
                              class="text-[10px] font-medium text-accent bg-accent/10 px-1.5 py-0.5 rounded"
                              >linked</span
                            >`
                          : nothing}
                      </div>
                        ...
                        ? html`
                            <div class="flex flex-wrap gap-1 shrink-0">
                              ${c.services.map(
                                (s) => html`
                                  <span
                                    class="text-[10px] bg-surface border border-border rounded px-1.5 py-0.5 font-mono text-text-muted"
                                    >${s.name}</span
                                  >
                                `,
                              )}
                              ...
                              ${!session
                                ? html`
                                    <button
                                      ...
                                      @click=${() =>
                                        manager.startSession(c.device_id)}
                                    >
                                      Connect
                                    </button>
                                  `
                                : html`
                                    <button
                                      ...
                                      @click=${() =>
                                        manager.closeSession(c.device_id)}
                                    >
                                      Disconnect
                                    </button>
                                  `}
  ...
  #renderSignaling() {
    ...
    return html`
      <div class="p-6 space-y-6">
                        ...
                        >
                          <div class="flex items-center gap-2">
                            ...
                            <span
                              ...
                              ${serverState.authError.reason}</span
                            >
                          </div>
                          ${serverState.authError.kind ||
                          serverState.authError.domain
                            ? html`
                                <div
                                  class="flex items-center gap-3 ml-6 text-text-muted"
                                >
                                  ${serverState.authError.kind
                                    ? html`<span
                                        >Auth:
                                        <code class="text-text font-mono"
                                          >${serverState.authError.kind}</code
                                        ></span
                                      >`
                                    : nothing}
                                  ${serverState.authError.domain
                                    ? html`<span
                                        >Domain:
                                        <a
                                          href="${serverState.authError.domain}"
                                          target="_blank"
                                          rel="noopener"
                                          class="text-accent hover:underline font-mono"
                                          >${serverState.authError.domain}</a
                                        ></span
                                      >`
                                    : nothing}
                                </div>
                  ...
                  ${serverState.cocoons.length > 0
                        ...
                        <div>
                          ...
                          <div class="space-y-1">
                            ${serverState.cocoons.map((c) => {
                              ...
                              return html`
                                <div
                                  class="flex items-center gap-3 px-3 py-2 bg-surface rounded border border-border/50"
                                >
                                  <span
                                    class="w-2 h-2 rounded-full shrink-0 ${c.status ===
                                    "online"
                                  ...
                                  <div class="flex-1 min-w-0">
                                    ...
                                    <p
                                      class="text-[10px] text-text-muted font-mono mt-0.5 truncate"
                                    >
                                      ${c.device_id}
                                    </p>
                                  </div>
                                    ...
                                    ? html`
                                        ...
                                        >
                                          ${c.services.map(
                                            (s) => html`
                                              <span
                                                class="text-[10px] bg-surface-alt border border-border rounded px-1.5 py-0.5 font-mono text-text-muted"
                                                >${s.name}</span
                                              >
                                            `,
                                          )}
                    ...
                    : html`
                        <p class="text-text-muted text-[10px]">
                          No cocoons discovered. Click "List Cocoons" to fetch.
                        </p>
                      `}
                </div>
              `;
        ...
        ${this.rtcSessions.size > 0
          ? html`
              <div>
                <h3
                  class="text-xs font-semibold text-text-muted uppercase tracking-wider mb-3"
                >
                  Active Sessions (${this.rtcSessions.size})
                </h3>
                <div class="space-y-1">
                  ${[...this.rtcSessions.values()].map(
                    (s) => html`
                      <div
                        class="flex items-center gap-3 px-3 py-2.5 bg-surface-alt rounded border border-border"
                      >
                        ...
                        <code
                          class="text-xs font-mono text-text truncate flex-1"
                          >${s.deviceId}</code
                        >
                        <code class="text-[10px] font-mono text-text-muted"
                          >${s.sessionId}</code
                        >
                      </div>
                    `,
    ...
    return html`
      <div class="p-6 space-y-6">
        ...
        ${this.registryDirty
          ? html`
              <div
                class="flex items-center justify-between px-3 py-2 bg-yellow-400/10 border border-yellow-400/30 rounded text-yellow-400 text-xs"
              >
        ...
        ${this.registries.length === 0
          ? html`<p class="text-text-muted text-sm">
              No registries configured. Add one above.
            </p>`
          : html`
                  ...
                  >
                    ...
                    <div class="flex items-center gap-3">
                      ${r.checking
                        ...
                        : r.health === null
                          ? html`<span
                              class="w-2.5 h-2.5 rounded-full bg-border shrink-0"
                            ></span>`
                          : r.health.online
                            ? html`<span
                                class="w-2.5 h-2.5 rounded-full bg-green-400 shrink-0"
                              ></span>`
                            : html`<span
                                class="w-2.5 h-2.5 rounded-full bg-red-400 shrink-0"
                              ></span>`}
                      <div class="flex-1 min-w-0">
                        <code class="text-xs font-mono text-text truncate block"
                          >${r.registry.url}</code
                      ...
                      ? html`
                          <div>
                            ...
                            <div class="grid grid-cols-3 gap-3">
                              <div>
                                <p
                                  class="text-[10px] text-text-muted uppercase tracking-wider mb-0.5"
                                >
                                  Status
                                </p>
                                <p
  ...
  #renderEventLog() {
    ...
    return html`
      <div class="flex flex-col h-full">
        <!-- Event log toolbar -->
        <div
          class="shrink-0 border-b border-border px-4 py-2 flex items-center gap-3"
        >
        ...
        <div class="flex-1 overflow-auto font-mono text-xs">
          ${rows.length === 0
            ? html`
                <div
                  class="flex flex-col items-center justify-center py-24 gap-2 text-text-muted"
                >
            ...
            : rows.map(
                (entry) => html`
                  <div
                    class="flex items-start border-b border-border/40 hover:bg-surface transition-colors"
                  >
                    <span
                      class=${[
                    ...
                    >
                    <span
                      class="shrink-0 w-56 py-2 text-accent font-bold truncate pr-4"
                      >${entry.event}</span
                    >
  ...
  override render() {
    return html`
      <div class="min-h-screen bg-bg">
        <!-- Header -->
        <div
          class="border-b border-border bg-surface px-6 py-4 flex items-center gap-3"
        >
          <svg
          ...
          >
            ...
            <path
              ...
              stroke-linejoin="round"
              stroke-width="1.5"
              d="M12 6V2m0 16v4M4.93 4.93l2.83 2.83m8.48 8.48 2.83 2.83M2 12h4m12 0h4M4.93 19.07l2.83-2.83m8.48-8.48 2.83-2.83"
            />
          </svg>
          <div>
            <h1 class="text-base font-semibold text-text">Debug Screen</h1>
            <p class="text-xs text-text-muted">
              App internals &amp; event monitor
            </p>
          </div>
          <div class="ml-auto">
            <button
