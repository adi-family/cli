...
// Shared state — populated by ActionsPlugin at startup, read by component on mount.
const store = {
...
export class AppActionsLoop extends LitElement {
  ...
  override connectedCallback(): void {
    ...
    const listener = () => { this.actions = store.actions; };
    ...
    return html`
      <div class="min-h-screen bg-bg p-6 space-y-1">
        <div class="mb-2">
          ...
          <p class="text-sm text-text-muted">${this.actions.length} pending action${this.actions.length !== 1 ? 's' : ''}</p>
        </div>
...
export class ActionsPlugin extends AdiPlugin {
  ...
  override onRegister(): void {
    this.bus.emit('route:register', { path: '/actions', element: 'app-actions-loop', label: 'Actions' }, 'actions-loop');
    this.bus.send('nav:add', { id: 'app.actions', label: 'Actions', path: '/actions' }, 'actions-loop').handle(() => {});

    this.bus.emit('command:register', { id: 'app:actions', label: 'Open Actions', shortcut: '⌘⇧A' }, 'actions-loop');
    this.bus.on('command:execute', ({ id }) => {
      if (id === 'app:actions') this.bus.emit('router:navigate', { path: '/actions' }, 'actions-loop');
    }, 'actions-loop');

    this.bus.on('actions:register-renderer', ({ plugin, kind, render }) => {
    ...
    this.bus.on('actions:dismiss', ({ id }) => {
      ...
      this.bus.emit('actions:dismissed', { id: card.id, plugin: card.plugin, kind: card.kind }, 'actions-loop');
      store.notify();
    }, 'actions-loop');
  }
}
